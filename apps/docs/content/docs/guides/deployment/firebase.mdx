...
2. Then, create two new web apps:
   - `transcript-seeker-api`: Replace `transcript-seeker-api-pre-prod` in `firebase.json` and `.firebaserc` with this new ID.
   - `transcript-seeker-proxy-pre-prod`: Replace `transcript-seeker-proxy-pre-prod` in `firebase.json` and `.firebaserc` with this new ID.
3. Navigate to **Hosting** and click "Get Started."
4. Finally, Replace `transcript-seeker-pre-prod` in `firebase.json` and `.firebaserc` with your project id. (This is for firebase hosting, when you click on get started it automatically creates a hosting project with ur app id)
